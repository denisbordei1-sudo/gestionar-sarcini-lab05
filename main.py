...
# ---------------------------------------------------------------------------
# Endpoint-uri generale și autentificare
# ---------------------------------------------------------------------------
...
def autentificare(
    ...
    utilizator = db.execute(
        "SELECT * FROM utilizatori WHERE email = ?", (formular.username.lower(),)
    ).fetchone()
...
def utilizatorul_curent(utilizator_curent=Depends(get_utilizator_curent)):
...
def actualizeaza_sarcina(
    ...
    db.execute(
        ...
        (titlu_nou, descriere_noua, finalizata_noua, sarcina_id, utilizator_curent["id"]),
    )
    db.commit()
    return dict(
        db.execute(
...
def finalizeaza_sarcina_logica(sarcina_id: int, db: sqlite3.Connection, utilizator_curent):
    sarcina = db.execute(
        "SELECT * FROM sarcini WHERE id = ? AND utilizator_id = ?",
        (sarcina_id, utilizator_curent["id"]),
    ).fetchone()
...
def finalizeaza_sarcina(
    ...
    return finalizeaza_sarcina_logica(sarcina_id, db, utilizator_curent)


# Alias inclus pentru compatibilitate cu varianta de endpoint scrisă în Laboratorul #04.
@app.patch("/sarcini/{sarcina_id}/finaliza")
def finaliza_sarcina_alias(
    sarcina_id: int,
    db: sqlite3.Connection = Depends(get_db),
    utilizator_curent=Depends(get_utilizator_curent),
):
...
# ---------------------------------------------------------------------------
# Servire frontend static - trebuie să fie ultimul rând
# ---------------------------------------------------------------------------
